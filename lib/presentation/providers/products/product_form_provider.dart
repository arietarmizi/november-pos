...
import '../../../app/services/google_drive_storage/google_drive_storage_service.dart';
...
class ProductFormProvider extends ChangeNotifier {
  ...
  Future<Result<int>> createProduct() async {
    try {
      if (imageFile != null) {
        cl("[createProduct] Uploading image...");
        // imageUrl = await FirebaseStorageService().uploadUserPhoto(imageFile!.path); uncomment if you want to use firebase storage instead of google drive
        imageUrl = await GoogleDriveOAuthService().uploadFile(imageFile!.path);

        if (imageUrl == null) {
          return Result.error(
            UnknownError(message: "Gagal upload gambar ke Google Drive"),
          );
        }
      }
